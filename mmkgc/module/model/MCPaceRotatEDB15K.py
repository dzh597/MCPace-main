...
class MCPaceRotatEDB15K(Model):
    ...
    def __init__(
        ...
        super(MCPaceRotatEDB15K, self).__init__(ent_tot, rel_tot)
        ...
        self.mcpace_context = None

    def _set_mcpace_context(
        self,
        batch_r,
        mode,
        h,
        ...
        """
        Save modal tensors used by the latest normal KGC forward pass.

        MCPace currently uses only the confirmed three modalities:
        structure, visual and textual. Numeric embeddings still participate in
        DB15K fusion/training as before, but they are not coordinated by MCPace.
        """
        self.mcpace_context = {
            "modalities": {
                "structure": (h, t),
                "visual": (h_img_emb, t_img_emb),
                "textual": (h_text_emb, t_text_emb),
            },
            "batch_r": batch_r,
            "mode": mode,
        }

    def get_mcpace_context(self):
        return self.mcpace_context
    ...
    def forward(self, data):
        ...
        self._set_mcpace_context(
            batch_r, mode, h, t, h_img_emb, t_img_emb, h_text_emb, t_text_emb
        )
